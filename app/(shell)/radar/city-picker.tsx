...
export function CityPicker({ uf, municipios, monitoradas = [] }: { uf: string; municipios: Municipio[]; monitoradas?: string[] }) {
  const [sel, setSel] = useState("");
  const disponiveis = municipios.filter((x) => !monitoradas.includes(x.codigo_ibge));
  const m = disponiveis.find((x) => x.codigo_ibge === sel);
  return (
    <form action={monitorarCidade} className="flex items-center gap-2">
      ...
      <Select value={sel} onChange={(e) => setSel(e.target.value)} className="h-8 w-48 text-xs" aria-label="Adicionar cidade">
        ...
        {disponiveis.map((x) => <option key={x.codigo_ibge} value={x.codigo_ibge}>{x.nome}</option>)}
      </Select>
